...
FilterSSSD <- "
model{
  N0 ~ dnorm(mu0,pa0)  ## initial conditions
  ## Forecast
  muN <- N0 + R*N0*(1-N0/K)
  muD <- 1
  D ~ dbern(p)
...
-   Figure on what happens when a SDA model that doesn't include disturbance encounters disturbance
-   More detail on what a model w/ disturbance but EnKF does during disturbance
-   Get the iterative filter working to see how it compares to the other two
