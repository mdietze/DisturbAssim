...
plot(time,n,type='l',ylim=range(n),xlim=xlim,ylab="LAI",lwd=3)
obs.sd = 0.75
n.true=n
n.obs = rnorm(NT,n.true,obs.sd)
points(time,n.obs,pch=18,col=2,cex=1.5)
legend("topright",legend=c("truth","obs"),lwd=c(2,NA),pch=c(NA,18),col=1:2)
...
## Foreward filter, separate process model

In the previous code, both the forecast and analysis steps were done within JAGS. In this version we're pulling out the Forecast step and running that in R since (i) that's how most models work and (ii) it will allow us to more easily distinguish the Forecast prior and Analysis posteriors
...
Xl = nIC         
pl = 1/n.sd^2
Na = rnorm(nMC,Xl,n.sd)
Nf = Na*NA

for(t in 1:NT){
  ## Forecast 
  d.prior <- rbinom(nMC,1,p.dist)   ## prior on disturbance status
  sel <- !as.logical(d.prior)
  Nf[!sel] <- rnorm(sum(d.prior),data$mu0,1/sqrt(data$pa0))  ## draw disturbed predictions
  Nf[sel]  <- rnorm(sum(sel),Na[sel] + r*Na[sel]*(1-Na[sel]/K),n.sd) ## draw undisturbed
  Nf[Nf < 0] <- 0 ## Tobit
  
  ## priors
  priors <- list(p    = mean(d.prior),
                 mufN = mean(Nf[sel]),
                 mufD = mean(Nf[!sel]),
                 pfN  = 1/var(Nf[sel]),
                 pfD  = 1/var(Nf[!sel])
  ...
  ## Analysis
  update = c(list(Y=data$Y[t],r=1/obs.sd^2),priors)
...
# plot(jdat)
  ...
  ## draw new IC
  Na <- sample(dat[,"n"],nMC,replace=TRUE)
...
### Save outputs required to include figure in Results
save(data,CI.fN,time,an0.stats,an.stats,NT,Dbar,xlim,col,file="SSSD.RData")
...
## EnKF
hist(LAI,probability = TRUE)#,ylim=c(0,dnorm(mua,mua,sqrt(Pa))))
...
hist(LAI,probability = TRUE,ylim=c(0,dnorm(mu1,mu1,sqrt(P1))))
...
#legend("top",legend=c("undisturbed","disturbed"),lwd=3,col=c("#b2df8a","brown"))
...
-   Add PFT switching
-   Apply to SIPNET SDA
    - version of code into PEcAn
    - test coupling to SIPNET
