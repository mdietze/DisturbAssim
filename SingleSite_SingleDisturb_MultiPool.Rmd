...
for(t in 1:NT){
  ...
  for(i in 1:ne){
    ...
    NmuK[i,] = VSEM(THETAK[i,],PAR = PAR[met[i],tsel])[365,2:4] ## deterministic simulation
  ...
  PaK <- (I-K %*% H)%*%PfK    
  paK = solve(PaK)
  NaK <- PEcAn.assim.sequential::adj.ens(Pf = PfK,X = NfK,mu.f = mufK,mu.a = muaK,Pa = PaK)
...
for(t in 1:NT){
  ...
  ## draw new IC     *** REPLACED WITH ENSEMBLE ADJUSTMENT ***
#  Na <- mvtnorm::rmvnorm(ne,mua,Pa)
  Na <- PEcAn.assim.sequential::adj.ens(Pf = Pf,X = Nf,mu.f = muf,mu.a = mua,Pa = Pa)
