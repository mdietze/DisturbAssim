...
PAR = tapply(flux$PPFD_IN,date.i,mean)*ein*86400*1e-12 ## µmolPhoton m-2 s-1 -> MJ /m2 /day
...
avg = c(mean(Cl,na.rm=TRUE),
            mean(Cw,na.rm=TRUE),
            mean(aNEE,na.rm=TRUE))

bias = c(mean((Cl.post-Cl),na.rm=TRUE),
            mean((Cw.post-Cw),na.rm=TRUE),
            mean((NEEm-aNEE),na.rm=TRUE))
bias
bias/avg
RMSE/avg

c(Ql,Qw,Qs)/c(avg[1:2],mean(Cs.post))
