...
**TODO Dongchen: consider adding a conceptual diagram to explain the framework and data**
...
** TODO from zoey for above figure: Labels could be more informative (i.e. EnKF = conventional and Multinomial = improved approach). Icon for "data" should match the icons rather than a line. Is the P(dist) bound at 0,1? I barely noticed the black line at first, it could maybe go in a second panel below with a shared x-axis and a distinct y-axis.**

** TODO from zoey for below figure: I think this figure could be more streamlined to better emphasize the difference between the filters. Why change the assimilation color from orange to blue in this figure?   The histogram, to me, seems more intuitive for displaying the data values rather than the forecasted values. The second panel doesn't have "forecast" in the color labels which also adds to confusion. The X axis could communicate the low-LAI = disturbed, high-LAI = undisturbed, without relying on the colors/legend. X axis for this and the Y axis for the above figure could just be "disturbance magnitude" **

```{r concept, echo=FALSE, message = FALSE,error=FALSE, fig.asp=1, fig.cap="Top: In a conventional EnKF, a multimodal forecast (gray histogram) is approximated by a unimodal Normal (green), such that when a low probability disturbance is actually observed (red) the posterior (orange) does not capture this event. Bottom: In the multinomial filter the same multimodal forecast is represented by a mixture of normals (green and dark red) each weighted by their prior (forecast) probabilities. The posterior (blue), updates the mean and variance of each mode as well as their relative probabilities, in this case capturing the high probability that a disturbance occurred."}
library(rjags)
par(mfrow=c(2,1))
## params
set.seed(1066)
...
where $\vec{\mu}_{f,k,t}$ and $P_{f,k,t}$ are the ensemble forecast ($f$) sample mean and covariance for outcome $k$ at time $t$. However, because we have $k$ categories we end up with a multimodal prediction (i.e., a mixture of MVNs). Furthermore, these modes need not be distinct; it is perfectly fine in concept for these MVNs to be overlapping. In practice the more they overlap the harder it is to identify which mode you are in, but as we will see this categorical assignment will be treated probabilistically. Indeed, to be able to represent our multimodal prediction as a single probability distribution we need to know the _forecasted_ probability of being in each category, which we'll take as known and represent using the vector $\vec{p_t}$, giving us a weighted MVN mixture as our forecast.
...
** TODO: Adjust fig 2 to make colors consistent between panels and with other figures**
...
** TODO: Check equation formatting **
...
Our fourth experiment was similar to the third, but extended the single-site, multi-disturbance, multi-pool scenario to one where one disturbance type included a switch in PFT. Specifically PFT 2 was identical to the VSEM default, but PFT 1 was parameterized with a stem residence time that was increased by 10x to create a more tree-like PFT. In addition, we established a transition matrix where disturbance 1 (stand clearing) transitions from PFT1 to PFT2, while disturbance 2 (defoliation) keeps the current PFT is. As with the previous scenario, the prescribed disturbance in the pseudodata was type 1 (stand clearing, transition from forest to shrub).
...
** TODO: TRY AGAIN AT GETTING MAPS ON SAME PROJECTION **
...
```{r map, echo=FALSE, fig.asp=1, fig.cap="Remotely sensed observations from central Oregon for 2004 used as inputs to test the multinomial filter against real-world disturbances"}
